...
nngp.inla<-function(formula, preprocess.data, mc.cores=1, family="gaussian", E=rep(1,dim(preprocess.data$data)[1]), offset = NULL,
                    phi.prior=c(1,30),sigma2.prior=c(2,1)){
  ...
  nngp.model <- inla.rgeneric.define(inla.rgeneric.nngp.model, neighbor.train=preprocess.data$nearest.neighbor ,M=preprocess.data$M,
                                     a=a,b=b,sigma2.prior=sigma2.prior,debug = T)
...
blocknngp.inla<-function(formula, preprocess.data, mc.cores=1, family="gaussian", E=rep(1,dim(preprocess.data$data)[1]), offset = NULL,
                    phi.prior=c(1,30),sigma2.prior=c(2,1)){
...
get.new.spatial<-function(n.sample=1,new.coord ,result, preprocess.data, model="nngp", phi.prior=c(1,30),
  ...
  inla.pos<-inla.posterior.sample(n=n.sample,result$res,num.threads = mc.cores,intern = FALSE,selection = list(idx=0))
    ...
    pre.spatial<- function(j="iterations",i="ith location"){
      ...
      if(sigma2-B.new%*%C_sn<0){ print("error:sigma2-B.new%*%C_sn<0");break}
...
spatial.plot<-function(coord=preprocess.data$unique.coord, values=result$res$summary.random$idx$mean, result, preprocess.data,
